...
__*Part (e) Is the underlying crop yield level $\mu_4$ obtained using the fourth fertilizer more than 0.5 units greater than the average of the underlying crop yield levels obtained using the other three fertilizers, $\mu_1$ , $\mu_2$ and $\mu_3$ respectively? *__
  ...
  To test $H_0$ we will run a mean paramterised version of our linear model run through a generalised linear hypothesis test along with a description of our null hypothesis.
```{r glht}
...
summary(ght)
```
  Here we can see a p-value of 0.0391 which allows us to reject our null hypothesis and conclude that the underlying crop yield for μ 4 is more than 0.5 units greater than the average underlying crop yield of the other three groups.
...
Bayesian_anova <- function(){
  ...
  # Priors - Uninformative with uniform variance
  m ~ dnorm(0.0, 0.0001)
  alpha[1] <- 0 
  ...
  # Tracking the means yield values
...
delta_alpha_model <- function(){
  ...
  # Priors - Uniformative with uniform variance
  m ~ dnorm(0.0, 0.0001)
  ...
  # Track value for Hypothesis test
  Alpha_four_test <- alpha[4] - ((alpha[1] + alpha[2] + alpha[3])/3)
}
```

  The output obtained is as follows:
```{r run_alpha_delta, include=FALSE}
