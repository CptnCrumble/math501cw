...
ggplot(data=df_long, aes(x=Fertilizer, y=Yield))+
  ...
  ggtitle("Crop yield grouped by fertilizer used upon the field")+
  ...
  coord_flip()
```
  Our visualisation suggests notable overlap between adjacent fertilizer groups and that *f1* could prove to be significantly different to groups *f3* and *f4*.
  

__*Part (b) Interpretation of the parameter $\alpha$*__

  For a given fertilizer group $\alpha$ represents the difference between the groups mean crop yield and the mean crop yield for group *f1*. If $\alpha$ = 0 for all of our groups then our data provides no evidence that these fertilisers have different effects on crop yield.
...
anova(yield_lm)
```
  Given that the size of this ANOVA test is 0.05 our p-value of 0.03388 means we can reject our null hypothesis and conclude that some of the observed difference in crop yield could be correlated to the fertilizer being used.
  
__*Part (d) Tukey Honest Significant Differences Test*__

  We will now compare inter-field crop yields using a Tukey Honest Significant Difference test. The null hypotheses for this test are:
...
```
  ...
  Whilst all groups have a non zero difference in mean yields only the difference between *f4* and *f1* passes our significance value of 0.05 so the only null hypothesis that we can reject is:
...
__*Part (e) Is the underlying crop yield level for _f4_ more than 0.5 units greater than the average of the underlying crop yield levels obtained using the other three fertilizers? *__
  ...
  To test $H_0$ we will run a mean parameterised version of our linear model run through a generalised linear hypothesis test along with a description of our null hypothesis, the results of which can be seen below.
```{r glht}
...
summary(ght)
```
  Here we can see a p-value of 0.0391 which allows us to reject our null hypothesis and conclude that the underlying crop yield for *f4* is more than 0.5 units greater than the average underlying crop yield of the other three groups.
...
Bayesian_anova_inference <- jags(data = data_anova, 
                                 ...
                                 n.iter = 100000, n.chains = 3, model.file = Bayesian_anova)
```
...
__*Part (j) Bayesian analysis of differences in $\alpha_f$*__
  ...
  We will use the following JAGS code to infer the posterior distributions for each of the hypotheses laid out in section (c) and (d):
...
delta_alpha_model <- function(){
  ...
  # Track value for part (d) Hypothesis
...
delta_alpha_inference <- jags(data = data_anova, 
                              ...
                              n.iter = 1000000, n.chains = 3, model.file = delta_alpha_model)
```
```{r print_results}
